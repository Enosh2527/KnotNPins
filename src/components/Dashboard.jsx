...
import { AnimatedContent, MagicBento, MagicBentoCard } from './MagicUI';
    ...
    <div className="animate-fade-in" style={{ position: 'relative' }}>
      ...
      <AnimatedContent delay={0.1}>
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', position: 'relative', zIndex: 1 }}>
            <div>
        ...
        <MagicBento>
          {[
            ...
            >
              ...
              <p style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)', margin: 0, marginBottom: '0.15rem' }}>{action.title}</p>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                ...
                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#8b8d95' }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#8b8d95' }} />
                <Tooltip 
                  contentStyle={{ background: '#1E1F24', border: '1px solid rgba(227,175,27,0.2)', borderRadius: '12px' }}
                  itemStyle={{ fontSize: '0.8rem' }}
                />
                <Area 
                  type="monotone" 
                  dataKey={chartMode} 
                  stroke={chartMode === 'revenue' ? '#3b82f6' : chartMode === 'profit' ? '#10b981' : '#ef4444'} 
                  fillOpacity={1} 
                  fill={`url(#g${chartMode.charAt(0).toUpperCase() + chartMode.slice(1)})`} 
                  strokeWidth={3} 
                />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div style={{ height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>No data for current filter</div>
          )}
        </div>
      </AnimatedContent>
              ...
              <div style={{ position: 'relative', width: '180px', height: '180px', flexShrink: 0, margin: '0 auto' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }} formatter={(v,name) => [`₹${v} (${expPie.find(e=>e.name===name)?.pct}%)`, name]} />
                  </PieChart>
                </ResponsiveContainer>
